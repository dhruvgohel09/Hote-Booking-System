...
$active_offers = [];
$offer_res = $mysqli->query("SELECT * FROM offers WHERE status='active'");
if ($offer_res) {
...
function get_room_discount($id, $offers) {
    foreach($offers as $o) {
        $tr = $o['target_rooms'];
        if (empty($tr) || in_array((string)$id, explode(',', $tr))) {
...
<div class="container mt-4 mb-2">
    <div class="alert alert-warning alert-dismissible fade show text-center shadow border-0" role="alert">
        <h4 class="alert-heading fw-bold mb-2 h-font"><i class="bi bi-tags-fill me-2 text-danger"></i>Flash Deals Available!</h4>
        <p class="mb-0 fs-5">
            <?php foreach($active_offers as $offer): ?>
                ...
                <span class="badge bg-danger ms-2 fs-6 px-3"><?php echo $offer['discount_percentage']; ?>% OFF</span><br>
            <?php endforeach; ?>
        </p>
...
<!-- Our Rooms -->
<h2 id="our-rooms" class="mt-5 pt-4 mb-4 text-center fw-bold h-font" style="scroll-margin-top: 100px;">OUR ROOMS</h2>
    ...
    <div class="row">
        ...
        <div class="col-lg-4 col-md-6 my-3">           
            <div class="card border-0 shadow position-relative" style="max-width: 350px; margin: auto;">
                <?php $d = get_room_discount(1, $active_offers); if($d > 0): ?>
                    <div class="position-absolute top-0 end-0 bg-danger text-white px-3 py-1 fw-bold fs-5 shadow text-center" style="border-radius: 0 0 0 15px; z-index: 5;">
                        <i class="bi bi-fire"></i> <?php echo $d; ?>% OFF
                    </div>
                <?php endif; ?>
        ...
        <div class="col-lg-4 col-md-6 my-3">           
            <div class="card border-0 shadow position-relative" style="max-width: 350px; margin: auto;">
                <?php $d = get_room_discount(2, $active_offers); if($d > 0): ?>
                    <div class="position-absolute top-0 end-0 bg-danger text-white px-3 py-1 fw-bold fs-5 shadow text-center" style="border-radius: 0 0 0 15px; z-index: 5;">
                        <i class="bi bi-fire"></i> <?php echo $d; ?>% OFF
                    </div>
                <?php endif; ?>
        ...
        <div class="col-lg-4 col-md-6 my-3">           
            <div class="card border-0 shadow position-relative" style="max-width: 350px; margin: auto;">
                <?php $d = get_room_discount(3, $active_offers); if($d > 0): ?>
                    <div class="position-absolute top-0 end-0 bg-danger text-white px-3 py-1 fw-bold fs-5 shadow text-center" style="border-radius: 0 0 0 15px; z-index: 5;">
                        <i class="bi bi-fire"></i> <?php echo $d; ?>% OFF
                    </div>
                <?php endif; ?>
...
<?php
$feedbacks = [];
$feedback_q = "SELECT f.rating, f.comments, CONCAT(u.first_name, ' ', u.last_name) AS name 
               FROM feedback f 
               INNER JOIN bookings b ON f.booking_id = b.id 
               INNER JOIN users u ON b.user_id = u.id 
               ORDER BY f.created_at DESC LIMIT 5";
$feedback_res = $mysqli->query($feedback_q);
if ($feedback_res) {
            ...
            <?php else: ?>
                <?php foreach($feedbacks as $fb): ?>
                    <div class="swiper-slide bg-white p-4">
                        <div class="profile d-flex align-items-center mb-3">
                            <img src="https://cdn-icons-png.flaticon.com/512/3135/3135715.png" width="40px" alt="User">
                            <h6 class="m-0 ms-2"><?php echo htmlspecialchars($fb['name']); ?></h6>
                        </div>
...
<script>
    ...
    function handleBookNow(roomId) {
        window.location.href = 'rooms.php?book=' + roomId;
    }
    ...
    function checkAvailability() {
        ...
        // Anyone can check availability, no login required. Redirect to rooms.php
        window.location.href = `rooms.php?checkin=${encodeURIComponent(checkin)}&checkout=${encodeURIComponent(checkout)}&adult=${encodeURIComponent(adult)}&children=${encodeURIComponent(children)}`;
    }
...
<?php if (!empty($active_offers)): ?>
<!-- Bootstrap Offer Modal -->
<div class="modal fade" id="offerModal" tabindex="-1" aria-labelledby="offerModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg text-center">
      <div class="modal-header bg-warning border-0 p-4">
        <h3 class="modal-title w-100 fw-bold h-font text-dark" id="offerModalLabel">
            <i class="bi bi-gift-fill me-2"></i>SPECIAL OFFERS!
        </h3>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body p-5">
        <?php foreach($active_offers as $offer): ?>
            <h4 class="text-danger fw-bold mb-3"><?php echo htmlspecialchars($offer['title']); ?></h4>
            <p class="fs-5 text-muted mb-4"><?php echo htmlspecialchars($offer['description']); ?></p>
            <div class="d-inline-block bg-danger text-white rounded-pill fs-3 px-5 py-2 mb-4 shadow">
                GRAB <?php echo (int)$offer['discount_percentage']; ?>% OFF
            </div>
            <?php if(count($active_offers) > 1) echo '<hr class="my-4">'; ?>
        <?php endforeach; ?>
        <p class="text-muted small mt-2">Valid for a limited time only. Applicable on select rooms natively during checkout.</p>
        <a href="#our-rooms" onclick="const m = bootstrap.Modal.getInstance(document.getElementById('offerModal')); if(m) m.hide();" class="btn btn-dark px-5 py-2 w-100 fw-bold rounded-pill shadow-none text-decoration-none">VIEW ROOMS</a>
      </div>
    </div>
  </div>
</div>
